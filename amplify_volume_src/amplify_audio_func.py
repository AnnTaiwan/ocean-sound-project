...
# New: turn up the volume
def amplify_audio(audio, factor=1.5):
    """
    Amplify the audio by a specified factor.
    
    Parameters:
        audio (numpy.ndarray): The audio signal.
        factor (float): The amplification factor (greater than 1 to increase volume).
        
    Returns:
        numpy.ndarray: The amplified and normalized audio signal.
    """
    amplified_audio = audio * factor
    # Normalize the audio to ensure it doesn't clip
    # can't exceed the range [-1,1]
    normalized_audio = amplified_audio / np.max(np.abs(amplified_audio))
...
def get_max_factor(audio):
    """
    calculate the max factor that audio can be amplified
    Parameters:
        audio (numpy.ndarray): audio samples。
        
    Returns:
        float: max amplify factor。
    """
    max_amplitude = np.max(np.abs(audio))
    if max_amplitude == 0:  # prevent divide zero
        return float('inf') # can't be amplified
