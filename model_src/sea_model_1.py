...
TRAINING_RESULT_FOLDER = "training_result_sea_model_1"
...
# define CNN model
class CNN_sea_model1(nn.Module):
    def __init__(self):
        super(CNN_sea_model1, self).__init__()
...
def training(model):
    # 把結果寫入檔案
    file = open(f"{TRAINING_RESULT_FOLDER}/training_detail_sea_model1_ver1.txt", "w")
    # 紀錄最大驗證集準確率
    max_accuracy = 0
    ...
    for epoch in range(NUM_EPOCHS):
        ...
        if(accuracy_valid > max_accuracy):
            ...
            if save_parameters:
                path = f"{TRAINING_RESULT_FOLDER}/sea_model1_ver1.pth"
    ...
    plt.savefig(f"{TRAINING_RESULT_FOLDER}/Confusion_matrix1_ver1.png") 
...
def plt_loss_accuracy_fig(Total_training_loss, Total_validation_loss, Total_training_accuracy, Total_validation_accuracy):
    ...
    plt.ylabel('Loss')
    plt.legend()
    plt.savefig(f"{TRAINING_RESULT_FOLDER}/Loss1_ver1.png") 
    ...
    plt.ylabel('Accuracy')
    plt.legend()
    plt.savefig(f"{TRAINING_RESULT_FOLDER}/Accuracy1_ver1.png") 
...
if __name__ == "__main__":
    ...
    model = CNN_sea_model1().to(device)
   
    # Load Images from a Folder
    image_folder_path = r"D:\ocean_sound_project\dataset_ver1\images\z_images_dataset_with_20db_ver1\train_mel_spec"
    ...
    # Load Images from a Folder
    image_folder_path = r"D:\ocean_sound_project\dataset_ver1\images\z_images_dataset_with_20db_ver1\val_mel_spec"
