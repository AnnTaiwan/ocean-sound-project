...
3. Test on image to see if consist of boat type
...
def generate_dataset(image_folder_path, batch_size=20, train_or_valid=True):
    ...
    # Load Labels
    # is_boat is 1, otherwise, it is 0
    labels = [1 if "boat" in os.path.basename(filename) else 0 for filename in image_paths]

    ### COUNT ###
    # 計算 1 的數量（包含 "boat" 的文件數量）
    num_boat_files = sum(labels)

    # 計算 0 的數量（不包含 "boat" 的文件數量）
    num_non_boat_files = len(labels) - num_boat_files

    print(f"Number of 'boat' files: {num_boat_files}")
    print(f"Number of non-'boat' files: {num_non_boat_files}")
    ########
...
# define CNN model
class CNN_model_boat_1(nn.Module):
    def __init__(self):
        super(CNN_model_boat_1, self).__init__()
...
def training(model):
    # 把結果寫入檔案
    file = open("training_result_model_boat_1/training_detail_boat_1.txt", "w")
    # 紀錄最大驗證集準確率
    max_accuracy = 0
    ...
    for epoch in range(NUM_EPOCHS):
        ...
        if(accuracy_valid > max_accuracy):
            ...
            if save_parameters:
                path = 'training_result_model_boat_1/model_boat_1.pth'
    ...
    plt.savefig("training_result_model_boat_1/boat_ROC1_ver1.png") 

    # confusion_matrix
    plt.figure()
    cm = confusion_matrix(all_label, all_pred)
    sns.heatmap(cm, annot=True)
    plt.savefig("training_result_model_boat_1/boat_Confusion_matrix1_ver1.png") 
...
def plt_loss_accuracy_fig(Total_training_loss, Total_validation_loss, Total_training_accuracy, Total_validation_accuracy):
    ...
    plt.ylabel('Loss')
    plt.legend()
    plt.savefig("training_result_model_boat_1/boat_Loss1_ver1.png") 
    ...
    plt.ylabel('Accuracy')
    plt.legend()
    plt.savefig("training_result_model_boat_1/boat_Accuracy1_ver1.png") 
...
if __name__ == "__main__":
    ...
    model = CNN_model_boat_1().to(device)
