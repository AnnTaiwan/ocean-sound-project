...
3. Test on image to see if consist of fish type
...
def generate_dataset(image_folder_path, batch_size=20, train_or_valid=True):
    ...
    # Load Labels
    # is_fish is 1, otherwise, it is 0
    labels = [1 if "fish" in os.path.basename(filename) else 0 for filename in image_paths]

    ### COUNT ###
    # 計算 1 的數量（包含 "fish" 的文件數量）
    num_fish_files = sum(labels)

    # 計算 0 的數量（不包含 "fish" 的文件數量）
    num_non_fish_files = len(labels) - num_fish_files

    print(f"Number of 'fish' files: {num_fish_files}")
    print(f"Number of non-'fish' files: {num_non_fish_files}")
    ########
...
# define CNN model
class CNN_model_fish_1(nn.Module):
    def __init__(self):
        super(CNN_model_fish_1, self).__init__()
...
def training(model):
    # 把結果寫入檔案
    file = open("training_result_model_fish_1/training_detail_fish_1.txt", "w")
    # 紀錄最大驗證集準確率
    max_accuracy = 0
    ...
    for epoch in range(NUM_EPOCHS):
        ...
        if(accuracy_valid > max_accuracy):
            ...
            if save_parameters:
                path = 'training_result_model_fish_1/model_fish_1.pth'
    ...
    plt.savefig("training_result_model_fish_1/fish_ROC1_ver1.png") 

    # confusion_matrix
    plt.figure()
    cm = confusion_matrix(all_label, all_pred)
    sns.heatmap(cm, annot=True)
    plt.savefig("training_result_model_fish_1/fish_Confusion_matrix1_ver1.png") 
...
def plt_loss_accuracy_fig(Total_training_loss, Total_validation_loss, Total_training_accuracy, Total_validation_accuracy):
    ...
    plt.ylabel('Loss')
    plt.legend()
    plt.savefig("training_result_model_fish_1/fish_Loss1_ver1.png") 
    ...
    plt.ylabel('Accuracy')
    plt.legend()
    plt.savefig("training_result_model_fish_1/fish_Accuracy1_ver1.png") 
...
if __name__ == "__main__":
    ...
    model = CNN_model_fish_1().to(device)
